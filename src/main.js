...
const displayFlag = (flagResponse) => {
  ...
  $('#currencyRate').on('click', function() {
    ...
    CurrencyService.getCurrency(fromCurrency, toCurrency)
      .then(function(currencyResponse) {
        ...
        return FlagService.getFlag(countryCode);
      })
      .then(function(flagResponse) {
        if (flagResponse instanceof Error) {
          throw Error(`Flag API Error: ${flagResponse.message}`)
        }
        displayFlag(flagResponse);
      })
